...
CREATE TABLE IF NOT EXISTS shifts (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT shifts_company_code_key UNIQUE (company_id, code)
);
...
CREATE TABLE IF NOT EXISTS leave_types (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT leave_types_company_code_key UNIQUE (company_id, code)
);
...
SELECT (SELECT id FROM companies WHERE code = 'DEFAULT'), name, code
FROM (VALUES
  ('Human Resources', 'HR'),
  ('Finance', 'FIN'),
...
SELECT (SELECT id FROM companies WHERE code = 'DEFAULT'), name, code, start_time::TIME, end_time::TIME, break_start::TIME, break_end::TIME, work_hours
FROM (VALUES
...
SELECT (SELECT id FROM companies WHERE code = 'DEFAULT'), name, code, quota_days, is_paid, requires_approval
FROM (VALUES
